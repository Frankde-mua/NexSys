  ...
  return (
    <div>
        ...
        <ul className="flex space-x-4">
          {tabs.map((tab, idx) => (
            <li key={tab} className="flex items-center">
              <button
                onClick={() => {
                  if (selectedClient || tab === "Invoice") {
                    setActiveTab(tab);
                  }
                }}
                className={`text-sm font-medium transition px-2 py-1 rounded-md ${activeTab === tab
                    ? "text-blue-600 font-semibold"
                    : "text-gray-600 hover:text-blue-600"
                  }`}
                  disabled={!selectedClient || Object.keys(selectedClient).length === 0}
              >
                {tab}
              </button>
      ...
      <div className="mt-6">
        ...
        {activeTab === "Payment" && (
          <Payment selectedClient={selectedClient} />
        )}

        {activeTab === "Enquiries" && (
          <FinancialEnquiries selectedClient={selectedClient} />
        )}

        {activeTab === "E-Transact" && (
          <ETransact selectedClient={selectedClient} />
        )}

        {activeTab === "Quote" && (
          <Quote selectedClient={selectedClient} />
        )}

        {activeTab === "Credit Note" && (
          <CreditNote selectedClient={selectedClient} />
        )}
